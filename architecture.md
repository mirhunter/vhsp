...
**The real, routinely-hit consequence of neither model being built yet**: since Traefik uses the HTTP-01 challenge (per DEPLOYMENT.md's Traefik setup), it requests each tenant's cert on the very first HTTPS request it sees for that hostname — which is often before a human has even seen, let alone pasted in, the suggested `A` record above. That first request fails outright, not just slowly. **Implemented as a visibility fix, not a code-level retry** (there's nothing to retry in code — Traefik already retries the ACME challenge itself, lazily, on the next HTTPS request for that hostname, once DNS actually is live): `dns_records.is_cert_live()` makes a direct TLS handshake to Traefik itself and lets `ssl`'s own CA/hostname verification distinguish a real Let's Encrypt cert from Traefik's self-signed fallback, surfaced on both the operator's and the tenant's DNS-records pages alongside the existing "Check records" button. This closes the "is my site actually broken, or just waiting on DNS" question this race raises, without resolving the underlying architectural gap (no automated DNS push, no DNS-01 path) that causes the race to exist at all. See the control-plane README's "DNS records and SSL certificate status" section.
