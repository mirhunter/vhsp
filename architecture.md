...
**The real, routinely-hit consequence of neither model being built yet**: since Traefik uses the HTTP-01 challenge (per DEPLOYMENT.md's Traefik setup) with the resolver set at the entrypoint (`--entrypoints.web.http.tls.certresolver=letsencrypt`), it orders each tenant's certificate the moment it discovers that tenant's routers — i.e. as the containers start, well before a human has seen, let alone pasted in, the suggested `A` record above. Those orders fail outright. **And they do not retry.** Observed on a live host: five failed orders inside sixteen seconds of tenant creation, then nothing for the following hour — including across repeated HTTPS requests to those hostnames after DNS had become correct, and across a restart of the tenant's own containers (identical labels, so Traefik sees no config change). The tenant stayed on Traefik's self-signed fallback until Traefik itself was restarted, which interrupts TLS for every tenant on the host. **Implemented so far as a visibility fix only**: `dns_records.is_cert_live()` makes a direct TLS handshake to Traefik and lets `ssl`'s own CA/hostname verification distinguish a real Let's Encrypt certificate from the fallback, surfaced on both the operator's and the tenant's DNS pages. That answers "is my site broken or just waiting on DNS", but it does not make an affected tenant recoverable without a platform-wide restart, and it does not resolve the underlying gap (no automated DNS push, no DNS-01 path) that causes the race. A per-tenant "reissue certificate now" action is the missing piece — see issue #18.
...
**First real single-public-IP test host:** the current dev environment's swarm Traefik (upstream of the VM entirely, and out of this project's control) can't provision TLS certs or route a brand-new domain without manual intervention, which surfaced concretely when adding `tenant1.example.com` — the swarm couldn't be made to issue a cert or route to it without hand-holding outside vhsp itself. Rather than keep fighting that upstream layer, the plan is to stand up a small budget VPS (a $6/mo DigitalOcean droplet — one public IPv4 plus one auto-configured IPv6 from its allotted /124, no NAT/firewall in front) for `tenant1.example.com` and let vhsp's own local Traefik be the only routing/TLS layer end to end, exactly as this section describes for eventual production. This also doubles as the first real test of cross-host tenant migration via the operator backup/restore feature (see "Backups per tenant" below) — taking a backup on the existing dev VM and restoring it onto the new droplet, rather than only having validated restore against a same-network test target.
